...
class LetterboxdScraper:
    ...
    def normalize_text(self, text: str) -> Set[str]:
        ...
        words = text.split()
        normalized_words = {word for word in words if word and word not in self.stopwords}
    ...
    def scrape_user_films(self, username: str, max_pages: int = 5) -> Dict:
        ...
        logger.info(f"Scraped {len(user_data['films'])} total films for {username}")
        ...
        for component in react_components:
            try:
                ...
                if year_match:
                    ...
                    film_data['title'] = item_name.replace(f' ({year_match.group(1)})', '').strip()
                else:
                ...
                if item_link:
                    film_data['url'] = urljoin('https://letterboxd.com', item_link)
                ...
                logger.debug(f"Error extracting film from React component: {e}")
                continue
    ...
    def _process_user_films(self, user_data: Dict) -> None:
        ...
        for film in user_data['films']:
            ...
            # In a full implementation, you might want to scrape film pages for director/country
...
class LetterboxdRecommender:
    ...
    def generate_recommendations(self, nyff_films: List[Dict], user_data: Dict, top_n: int = 5) -> List[Dict]:
        ...
        """
        logger.info(f"Generating recommendations for {len(nyff_films)} NYFF films")
        ...
        for i, rec in enumerate(recommendations, 1):
            logger.info(f"#{i}: {rec['film']['title']} (score: {rec['score']}) - {rec['reasoning']}")
    ...
    def _score_film(self, film: Dict, user_data: Dict) -> Tuple[int, str]:
        ...
        if film_director and film_director in user_data['directors']:
            ...
            reasoning_parts.append(f"Director {film_director} ({count} films watched)")
        ...
        if film_country and film_country in user_data['countries']:
            ...
            reasoning_parts.append(f"Country {film_country} ({count} films watched)")
        ...
        if film_description:
            ...
            if keyword_overlap >= 5:
                score += 2
                reasoning_parts.append(f"High keyword similarity ({keyword_overlap} matches)")
            elif keyword_overlap >= 3:
                score += 1
                reasoning_parts.append(f"Moderate keyword similarity ({keyword_overlap} matches)")
        ...
        if film_director and user_data.get('ratings'):
            ...
            if director_ratings:
                ...
                if avg_rating >= 4:  # High ratings (assuming 1-5 scale)
                    score += 1
                    reasoning_parts.append(f"Highly rated director (avg: {avg_rating:.1f}/5)")
        
        reasoning = "; ".join(reasoning_parts) if reasoning_parts else "Basic compatibility"
        return score, reasoning


def get_letterboxd_recommendations(nyff_films: List[Dict], username: str, 
                                 cache_dir: str = "cache", top_n: int = 5) -> Optional[List[Dict]]:
    ...
    try:
        ...
        recommendations = recommender.generate_recommendations(nyff_films, user_data, top_n)
